...
class M_Preorder extends CI_Model {
    ...
    function Process($no_po) {
        $exec = $this->db->select('penawaran.status_quotation,customers.nama,customers.perusahaan,customers.alamat_perusahaan,customers.telepon,customers.mail,preorder.no_po,preorder.tgl_po,product.`name` AS nama_barang,product.partnumber,preorder.qty,product.price,( SELECT SUM( preorder.qty * product.price ) FROM preorder INNER JOIN product ON preorder.nama_barang = product.id WHERE preorder.no_po = ' . $no_po . ') AS total,penawaran.no_penawaran,penawaran.tgl AS tgl_penawaran')
                ->from('customers')
